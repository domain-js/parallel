...
function Parallel(cnf, deps) {
    ...
    const { async, logger, graceful, U: { sleep }, redis } = deps;
    // 存放当前处于执行中的 key
    ...
    const control = (method, opt) => {
        let { path, // 必选 并发控制key的主路径
        ...
        const paralleled = async (...args) => {
            ...
            if (!size) {
                ...
                // 需要等待
                await async.whilst(async () => Boolean(await redis.hexists(KEY, key)), async () => sleep(needWaitMS));
